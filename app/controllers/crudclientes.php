...
//MODIFICAR: Se puede simplifcar bastante: 
...
function crudPostAlta()
{
    ...
    //comprobamos que no se repita el mail.
...
function crudPostModificar()
{
    ...
    // Si el correo es diferente al correo que tiene ese ID
    ...
    if ($db->modCliente($cli)) {
        // Si la modificación es exitosa, subir la foto con el formato deseado
...
function subircambiarFoto($id)
{
    ...
    $maxFileSize = 500 * 1024; // 500KB en bytes
    if ($fileSize > $maxFileSize) {
        $_SESSION['msg'] = "El tamaño del archivo excede el límite permitido (500KB).";
        return false;
    }

    // Generar el nombre del archivo con el formato deseado (8 dígitos)
    $fileNameWithFormat = sprintf("%08d", $id) . ".$fileExtension";
    $uploadDirectory = "app/uploads/";

    // Mover el archivo a la carpeta uploads con el nombre generado
